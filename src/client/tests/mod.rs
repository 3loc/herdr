...
#[test]
fn terminal_app_resize_command_maps_to_pty_geometry() {
    let action = terminal_app_command_from_json(
        r#"{"type":"terminal.resize","cols":101,"rows":37,"cell_width_px":9,"cell_height_px":18}"#,
    )
    .unwrap();
    let TerminalAppCommand::Resize(size) = action else {
        panic!("expected PTY resize");
    };
    assert_eq!(size.cols, 101);
    assert_eq!(size.rows, 37);
    assert_eq!(size.pixel_width, 9);
    assert_eq!(size.pixel_height, 18);
}

#[test]
fn terminal_session_mouse_mode_enables_bridge_clicks() {
