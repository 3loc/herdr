...
`terminal.resize`, and `terminal.release` records. The Herdr server remains the
owner of application state, keybindings, pane navigation, rendering, and the
terminal processes; the bridge only carries the rendered app stream.
