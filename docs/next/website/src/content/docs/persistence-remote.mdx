...
To bridge the complete Herdr interface instead of one pane, use the writable
full-app controller:

```bash
herdr terminal session app --cols 120 --rows 40
```

It uses the same newline-delimited `terminal.frame`, `terminal.input`,
`terminal.resize`, and `terminal.release` records. Herdr runs its normal client
shell in an isolated PTY, so the server remains the owner of shared session and
terminal state while the bridge carries the client-rendered app stream.
