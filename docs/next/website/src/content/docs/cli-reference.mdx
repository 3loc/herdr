...
herdr pane notes list <pane_id>|--current
herdr pane notes add <pane_id>|--current <text>
herdr pane notes remove <pane_id>|--current <index>
herdr pane notes clear <pane_id>|--current
...
Pane notes are short lines a user stacks against a pane while its agent is busy. Herdr delivers one queued note per completed agent turn, so they usually reach the agent as ordinary prompts; `pane notes list` reads them directly when an agent needs the standing context behind a request. Notes also appear as `notes` on `pane get` and `pane list`. Each pane holds up to 100 notes of 4 KiB each.
...
herdr terminal session app [--cols N] [--rows N]
...
`terminal session app` opens a writable live ANSI stream for the complete Herdr
interface. It uses the same newline-delimited input, resize, release, frame and
closed records as a terminal session controller, while Herdr keeps ownership of
the full application state and rendering.
