...
Press `prefix+h` to focus the active workspace row in the desktop sidebar,
regardless of which pane is focused. Use `j`/`k` to move through workspace and
agent rows, then `l` or Enter to activate the selection and return to its pane.
A hidden sidebar is not keyboard-focusable; the compact sidebar remains
available.
...
| Copy mode | `prefix+[` |
| Note a thought for the pane | `prefix+a` |
| Open the pane's queued notes | `prefix+m` |
