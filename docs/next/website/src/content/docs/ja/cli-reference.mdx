...
ペインメモは、エージェントの作業中にユーザーが追加できる短い指示です。Herdr は完了したターンごとにキューのメモを 1 件送信します。`pane notes list` でメモを直接確認でき、`pane get` と `pane list` の `notes` にも表示されます。各ペインには 4 KiB 以下のメモを最大 100 件保存できます。
